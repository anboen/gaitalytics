...
from gaitalytics.input import EzC3dFileHandler, TrcPointInputFileReader

INPUT_FILE_C3D: Path = Path('tests/data/test_healthy.c3d')
INPUT_FILE_LONG_C3D: Path = Path('tests/data/Baseline.5.c3d')
OUTPUT_FILE: Path = Path('out/test_healthy.hdf5')
OUTPUT_FILE_LONG: Path = Path('out/Baseline.5.hdf5')

INPUT_FILE_TRC: Path = Path('tests/data/subject01.trc')
...
def test_load_directions():
    c3d = EzC3dFileHandler(INPUT_FILE_C3D)
...
def test_load_heavy_event_c3d():
    c3d = EzC3dFileHandler(INPUT_FILE_LONG_C3D)
...
def test_load_point_trc():
    trc = TrcPointInputFileReader(INPUT_FILE_TRC)
    print(trc.get_size())
    assert False
