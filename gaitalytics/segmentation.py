...
class GaitEventsSegmentation(_BaseSegmentation):
    ...
    def _get_times_of_events(self, events: pd.DataFrame) -> dict[str, list[int]]:
        ...
        Returns:
            dict[str, list[int]]: A dictionary containing the contexts as keys and the event times as values.
        """
        splits = {}
        interesting_events = events[events[io.EventInputFileReader.COLUMN_LABEL] == self.event_label]
        contexts = events[io.EventInputFileReader.COLUMN_CONTEXT].unique()
        for context in contexts:
            context_events = interesting_events[interesting_events[io.EventInputFileReader.COLUMN_CONTEXT] == context]
    ...
    def _update_attrs(segment: xr.DataArray, cycle_id: int, context: str):
        ...
        start_frame = round(segment.coords['time'][0].data / (1 / segment.attrs['rate']), 0)
        end_frame = round(segment.coords['time'][-1].data / (1 / segment.attrs['rate']), 0)
